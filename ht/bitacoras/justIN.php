<?php
    if($eventNoComunes=="no" && $okFechas=="no")
    {
        ...
        $sql="SELECT * FROM bitacora_justificacion";
    ...
    {
        //Mostrar TODA la bitácora
        $sql="SELECT * FROM bitacora_justificacion where fecha between '$finicio' AND '$ffin'";
        $query=mysqli_query($con, $sql);
        if(!$query)
        {
            echo "Error al recuperar los datos de la bitácora.";
    ...
    {
        $sql="SELECT * FROM bitacora_justificacion where operacion='Actualizado' or operacion='Eliminado'";
        $query=mysqli_query($con, $sql);
        if(!$query)
        {
            echo "Error al recuperar los datos de la bitácora.";
