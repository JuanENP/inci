...
session_start();
    ...
    else
    {
        if($_POST["opcion"]=="indiv")
        {
            if (!empty($_POST["num"]) && !empty($_POST["fec"]))
            {
                ...
                //Ver en qué periodo está la fecha que llegó
                $periodoFec=obtenPeriodoDeFecha($fec);

                //Evitar que se puedan agregar fechas a periodos ya pasados
                if(($periodo==$periodoFec) || ($periodoFec<$periodo))
                {
                    ...
                    //saber cuantos pt tiene este empleado  y sumárlos a los días merecidos__NO. LOS PT NO SE SUMAN ACÁ.
                    $diasRestantes=$dias_merecidos-(int)$tomados;
                    if($diasRestantes>0)
                    {
                        $sql="INSERT INTO dias_vacaciones (dia, periodo, tomado, vacaciones_vacaciones) VALUES ('$fec', $periodo, 0, $idVac)";
                        if($query=mysqli_query($con, $sql))
                        {
                            echo "<script> imprime('Día de vacación agregado correctamente ' +
                            'al trabajador con número $num.'); </script>";
                        }
                        else
                        {
                            echo "<script> imprime('Ha ocurrido un error al insertar el día de vacación. Reintente...'); </script>";
                        }
                    }
                    else
                    {
                        echo "<script> imprime('El empleado con número $num ya ha tomado todos sus días de vacaciones ' +
                        'permitidos ($dias_merecidos días). No es posible agregar este día.'); </script>";
                    }
                }
                else
                {
                    echo "<script> imprime('No es posible agregar un día de vacaciones a un periodo anterior. ' +
                    'Esta fecha no se agregó al trabajador con número $num por el motivo anterior.'); </script>";
                }
            }
            else
            {
                echo "<script> imprime('No ha elegido un número de trabajador ' +
                'válido o una fecha'); </script>";
            }
        }
    ...
    function obtenPeriodo()
    {
        ...
        //periodo 2: del 1 julio al 31 diciembre
    ...
    function obtenPeriodoDeFecha($fecha)
    {
        /*
            La función revisa en qué periodo está la fecha que recibe y devuelve dicho periodo
        */
        $year=date("Y");
    ...
    //Fin función obtenPeriodoDeFecha
